...
public class HEBill extends Record {
	...
	protected int GetTranslate(StringBuffer resbuf, String mobileNo) throws IOException {
		...
		while ((end = buf.indexOf("\r\n", start)) != -1) {
			...
			if (value_bill.length >= 24) { // 数据头长度为24项
				...
				resbuf.append("ACCTID^"+value_bill[0]+"|"+"MOBILENO^"+value_bill[2]+"|BRAND^"+value_bill[3]+"|BILLCYC^"+value_bill[4]
					+"|BILLSTART^"+value_bill[5]+"|BILLEND^"+value_bill[6]+"|BILLTYPE^"+value_bill[19]+"|VERSION^"+value_bill[20]+"|");

				for (int i = 24; i < value_bill.length; i++) {
					String[] acctDetail = value_bill[i].split("\\^");
					if (acctDetail.length == 3) {
						// 补充"入账时间~预存活动~类别~金额~备注~待返还月份数~待返还金额"，待返还月份数~待返还金额如没有则填充空
						// 补充"赠送时间~赠送活动~类别~金额~备注~待赠送月份数~待赠送金额"，待赠送月份数~待赠送金额如没有则填充空
						if(acctDetail[2].startsWith("091")) {
							String[] notYetDonate = acctDetail[1].split("~", -1);
							for (int j = notYetDonate.length; j < 7; j++) {
								acctDetail[1] += "~";
							}
