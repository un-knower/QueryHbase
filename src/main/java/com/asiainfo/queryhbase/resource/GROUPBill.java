...
public class GROUPBill extends Record {
	...
	public StringBuffer GetResult(HeadMessage head, HashMap<String, String> map) {
		...
		logger.info("号码：" + mobileNo + "，开始时间：" + startCycle + "，结束时间：" + endCycle);
		...
		try {
			...
			try {
				...
				logger.error("集团账单查询，时间格式有误，startCycle=" + startCycle + "，endCycle=" + endCycle);
				resbuf.setLength(0);
				head.setErrcode("3");
			...
			logger.info("实际的开始时间：" + startCycle + "，结束时间：" + endCycle);
